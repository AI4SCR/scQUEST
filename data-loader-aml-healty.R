...
level = 'Levine_CyTOF_AML_Healthy_vs_AML'

files = dir(paste(root, level, sep='/'))
files.fcs = files[grepl('.fcs$', files)]

# mappings
rename.sample = readr::read_csv(paste(root, level, 'Levine_Healthy_vs_AML_sampleID.csv', sep='/'))
rename.sample  %<>% rename(sample_id = sample_ID)
map.sample = mapper('sample_id', rename.sample)

rename.channel = readr::read_csv(paste(root, level, 'Levine_Healthy_vs_AML_channels.csv', sep='/'))
...
anndata::write_h5ad(ad, paste(root, level, 'ad_healty_aml.h5ad', sep='/'))
