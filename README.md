...
If you don't have a very good reason to hide your code, it's recommended to allow internal access a over a private repo.

### Adapting the blueprint for you: TODOs

Of course, clone your repo.

- [ ] [.travis.yml](.travis.yml) notifications: delete the section if undesired, else set up a slack channel and set the token. Go to [travis](https://ibm-research.slack.com/apps/new/A0F81FP4N-travis-ci) and create a token for the channel you want to post into.
...
- [ ] choose a new name to replace "blueprint" as package name. If really needed, underscores are valid. Note that the repo name is independent. You can apply the naming changes using [this bash script published as gist](https://gist.github.ibm.com/DOW/3089cd08c16731a3d39c56af64265d54).
...
```
#### Packaging
See [https://packaging.python.org/guides/distributing-packages-using-setuptools/](https://packaging.python.org/guides/distributing-packages-using-setuptools/) to read up on python packaging with a `setup.py`.  
Note that there are other systems to package your code, e.g. `poetry`.
...
The `py.typed` file in the package directory is only there to mark the package to support typing.
...
Have a look a the example [slack channel for the blueprint bots](https://ibm-research.slack.com/archives/C02167RH2LW). In case you create a dedicated channel for bots of the project, another interesting one is the github bot that notifies on issues, PRs, etc.
...
There is also a `.dockerignore` file to prevent directories being copied over.  
...
See [here](https://taas.cloud.ibm.com/tools/artifactory).
...
The same story applies similarly to github tokens using https authentication; if you pass them as build arguments they are exposed.

### Licence header
Working on IBM internal code, you might be required to add a licence header to each of your source files. It's easier to add it from the start than to add it afterwards. For open sourcing, deleting will not be hard to do either.
In python we can add the special variable `__copyright__ ` after the module docstring and before the imports (with exception of `__future__` imports) like so:
...
"""
```
Creating a snipped for this in your IDE might be useful.